...
function batchScan (req, res, next) {
  ...
  if (result.error) {
    logger.error(`ClamAVController->batchScan(): validationg error ${result.error}`)
  ...
  logger.debug(`ClamAVController->batchScan(): posting unscanned event to the event bus: ${JSON.stringify(reqBody)}`)
